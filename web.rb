      ...
      when Line::Bot::Event::MessageType::Text
        message = case event.message['text']
                  ...
                  when /所以我說那個.+呢/i, /醬汁呢/i
                    result = event.message['text'].match(/所以我說那個(.+)呢/i)
                    word = result ? result[1] : '醬汁'
                    {
                      text: JIANGZHINE.sample % %W(#{word} #{word} #{word}),
                      type: 'text'
                    }
                  when /作弊|泡泡|陳世彥/
                    {
                  ...
                  when /武德|wuder|億全|波斯|boss|好笑嗎|好秀嗎|eggsy/i
                    {
                      text: BOSS_SAY.sample,
                      type: 'text'
                    }
                  when /大哥|翻譯|子彈|兄弟|縣長|市長/
                    {
                      text: BULLETS.sample,
                      type: 'text'
                    }
                  when /你的錯|怪我囉?|都你|說清楚|解釋/
                    {
                      text: ['大人，冤枉啊!', '怪我囉(╯⊙ ⊱⊙╰　)'].sample,
                      type: 'text'
                    }
                  when /matt|radar/i
                    {
                  ...
                  when /(他|她)媽的/, /^幹\s?.+/
                    {
                      text: ['對不起,下次不敢了', '阿扁錯了嗎?', '有話好說!', '猴，沒收功就說髒話', '幹，單挑啊( #｀Д´)'].sample,
                      type: 'text'
                    }
                  when /李嚴|帥哥|中華/, /(小|洨|笅)當家/, /[餓吃食廚]/i
                    {
                      text: ['你怎麼帥成這樣', '呵呵呵呵呵哈哈哈', '母親的味道，我的身體最知道了！', '就是媽媽的味道', '我是個廚師', '向恩，多虧你～＼(^o^)／', '就讓我送你到羅歇那去吧', '你們ㄘㄘ看', '很好ㄘヾ(*´∀｀*)ﾉ', '我的兒子會掉到山谷底下', '你們幾個給我聽好了', '應該還有用那個吧', '是那個吧', '都是你害我全身濕成這個樣子', '沒有完成的料理，根本沒有必要試吃!'].sample,
                      type: 'text'
                    }
                  end
...
BOSS_SAY = %w(好秀嗎？ wu你妹 億全 愛溝共 小組抱抱 西咧靠腰 衝啥 亞榆勒 大家好!我叫陳囧)
...
MATT_SAY =  ['Exactly..Exactly', '那我也不想給你看' , 'Hi,I\'m Matt.', '你們喜歡在這工作嗎?', '你們有看過凱羅忍的光劍嗎?', '你們覺得凱羅忍怎麼樣?', '你們相信他能像他所說的完成達斯.維達的遺願嗎?', '你這樣我壓力很大!', '嘿, 你踢我的扳手', '才不是, 它超屌的', 'After Rain, Comes the Rainbow', '我看我能不能找到, 讓你們見識一下', '你們看！我找到凱羅忍的光劍了', '他說凱羅忍有八塊肌, 說他身材超健美', '我能知道你在想什麼', '你的想法..很愚蠢!!', '噢不, 他被食物噎到了!', '我想吃瑪芬蛋糕!', '別叫了，機器人也是會累的..' ]
